...
bool virt_mouse::relay_mouse_event(struct input_event ev) {
    // check event code
    switch (ev.code) {
    case ABS_RX:
    ...
    case BTN_TR2:
        ...
        break;
    case BTN_TL2:
        // pass left trigger as right click
        libevdev_uinput_write_event(uidev, EV_KEY, BTN_RIGHT, ev.value);
        libevdev_uinput_write_event(uidev, EV_SYN, SYN_REPORT, 0);
        break;
    default:
        return false; // no we did not consume
    }
...
void *virt_mouse::__mouseLoop(void *args) {
    ...
    while (self->ready.load()) {
        float _axis_x_update = self->axis_x.load();
        ...
        // don't infinitely write 0;
        if (_axis_x != 0 && _axis_y != 0 && _axis_x_update != 0 && _axis_y_update != 0) {
            _axis_x = _axis_x_update;
            _axis_y = _axis_y_update;
