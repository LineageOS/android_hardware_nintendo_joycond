...
::ndk::ScopedAStatus Joycond::setSense(bool right, float sense) {
    ...
    if (right) {
        ...
        SetProperty(PROP_RS_SENSE, std::to_string(sense));
    } else {
        mMapping.sense_l = sense;
        SetProperty(PROP_LS_SENSE, std::to_string(sense));
    }
    pthread_mutex_unlock(&mapLock);
...
::ndk::ScopedAStatus Joycond::setDead(bool right, float dead) {
    ...
    if (right) {
        ...
        SetProperty(PROP_RS_DEAD, std::to_string(dead));
    } else {
        mMapping.dead_l = dead;
        SetProperty(PROP_LS_DEAD, std::to_string(dead));
    }
    pthread_mutex_unlock(&mapLock);
...
::ndk::ScopedAStatus Joycond::setLimit(bool right, float limit) {
    ...
    if (right) {
        ...
        SetProperty(PROP_RS_LIMIT, std::to_string(limit));
    } else {
        mMapping.limit_l = limit;
        SetProperty(PROP_LS_LIMIT, std::to_string(limit));
    }
    pthread_mutex_unlock(&mapLock);
