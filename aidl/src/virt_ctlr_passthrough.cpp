...
void virt_ctlr_passthrough::handle_events(int fd) { phys->handle_events(); }

bool virt_ctlr_passthrough::contains_phys_ctlr(
...
std::vector<std::shared_ptr<phys_ctlr>>
virt_ctlr_passthrough::get_phys_ctlrs() {
    std::vector<std::shared_ptr<phys_ctlr>> ctlrs = {phys};
    return ctlrs;
}

void virt_ctlr_passthrough::remove_phys_ctlr(
    const std::shared_ptr<phys_ctlr> phys) {
    ALOGE("Cannot remove phys_ctlr from a passthrough controller\n");
    exit(EXIT_FAILURE);
}

void virt_ctlr_passthrough::add_phys_ctlr(std::shared_ptr<phys_ctlr> phys) {
    ALOGE("Cannot add phys_ctlr to a passthrough controller\n");
    exit(EXIT_FAILURE);
}
